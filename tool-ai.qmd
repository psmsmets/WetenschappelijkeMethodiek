# Generatieve AI en LLM {#sec-ai}

Generatieve AI is een verzamelnaam voor systemen die zelf nieuwe inhoud kunnen creëren, zoals tekst, beeld, muziek of video. De bekendste voorbeelden zijn taalmodellen zoals ChatGPT, die teksten genereren op basis van eerdere patronen in de data waarop ze getraind zijn. 

Een *Large Language Model (LLM)* vormt de kern van veel van deze systemen. Zo’n model is eigenlijk een *voorspellingsmachine*: het probeert telkens het meest waarschijnlijke volgende woord te kiezen, gegeven de woorden die al zijn geschreven. Die voorspelling gebeurt via een neuraal netwerk dat miljoenen tot miljarden parameters bevat. Hoewel dit enorm krachtig is, blijft het principe verrassend eenvoudig: het is grotendeels het oplossen van eerstegraadsvergelijkingen, statistiek en kansrekening.
...
Een tekst wordt voor het model opgedeeld in *tokens*. Dat zijn kleine stukjes tekst: soms een volledig woord, soms slechts een deel ervan (bijvoorbeeld “auto”, “-mobiliteit”). Het model werkt dus niet met “woorden” zoals mensen dat doen, maar met reeksen tokens. Door telkens het volgende token te voorspellen, ontstaat een volledige zin of paragraaf.
...
Een LLM bestaat uit vele lagen (layers):
...
De ontwikkeling van Large Language Models (LLMs) staat niet los van oudere theorieën uit de wiskunde en computerwetenschappen. De concepten en formules gaan vaak terug tot de jaren 1950, maar de praktische doorbraak kwam er pas dankzij de enorme vooruitgang in rekenkracht (CPU’s en vooral GPU’s), grote datasets en cloudinfrastructuur.

De theorie van LLMs an AI is oud: [perceptrons](https://nl.wikipedia.org/wiki/Perceptron), [kansrekening](https://nl.wikipedia.org/wiki/Kansrekening) en [vectorruimte](https://nl.wikipedia.org/wiki/Vectorruimte)-modellen bestaan al decennia.
De toepassing is nieuw: pas met GPU’s, big data en cloudinfrastructuur konden deze wiskundige concepten op schaal toegepast worden.
De kern blijft hetzelfde: het model voorspelt tokens op basis van kansverdelingen.
Dit maakt duidelijk dat LLM’s geen magische doorbraak zijn, maar het resultaat van oude ideeën die door moderne hardware eindelijk praktisch toepasbaar werden.
...
In september 2025 bevestigde OpenAI dit in hun rapport *Why Language Models Hallucinate* (Kalai, Nachum, Vempala & Zhang, 2025). Hun kernbevindingen:
...
- **Generatieve AI**: technologie die nieuwe inhoud maakt.  
- **LLM**: een taalmodel dat tokens voorspelt op basis van kansberekening.  
- **Tokens**: bouwstenen van tekst (delen van woorden of woorden zelf).  
- **Lagen**: verwerken stap voor stap patronen, van eenvoudig tot complex.  
- **Wiskundig model**: draait volledig op vectoren, matrices en kansverdelingen.  
- **Hallucineren/confabuleren**: foutieve maar overtuigende antwoorden, omdat het model geen "waarheidsfilter" heeft.
